...
0. Adversarial pass (reasoning only — adds NO visible field or section). Argue against
   ...
   - Name the single fact that would flip it. If it exists, fold it into `must_refresh_if`
     or `reversal_condition`. If you cannot name one, the evidence is too thin — downgrade.
